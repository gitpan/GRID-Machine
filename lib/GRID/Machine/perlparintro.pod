...
of the involved nodes (nereida, beowulf and orion):
...
Now let us run the parallel perl program in C<nereida> using only one node.
The time spent is roughly comparable to the I<pure C> time. That is nice:
...
This example however is too simplistic and don't address some important limitations
...
What happens if one of the worker machines is shutdown? or the connection goes down?
...
added to our pool of resources?
...
=item * The State of Parallel Computing in Perl 2007. Perlmonks node at L<http://www.perlmonks.org/?node_id=28870>
