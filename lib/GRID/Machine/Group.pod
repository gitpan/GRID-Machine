...
Since our goal is to optimize the execution time, we distribute
the sum in line 6 C<$area = sum(map { f($_)/$N } @halves)> among
the processors. The machines are numbered from 0 to C<np-1> 
(being C<np> the number of machines) and each machine sums up
the areas of roughly C<N/np> intervals. 
