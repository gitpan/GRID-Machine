...
of the user's I<publick key>. To achieve this automatic ssh-authentification
you have to:
...
The C<-N> option is followd by the I<passphrase>. The C<-N ''> setting
...
to the C<home/user/.ssh/authorized_keys> file on the remote machine.
...
See the section L<EXAMPLE> for a full example.
...
to be better to have the code in some separated module.
...
machine to the remote machine using C<modput>.
...
C<include> will load C<List::Util> on the remote machine importing
the C<sum> function.
...
with the C<GRID::Machine> distribution.
...
to set the connection and shares its limitations. The following program produces an error:
